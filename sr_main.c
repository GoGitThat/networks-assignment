...
int main(int argc, char **argv)
{
    ...
    char *logfile = 0;
    int useNat = 0;
    unsigned int icmp_timeout_expiry = DEFAULT_ICMP_TIMEOUT;
    unsigned int tcp_established_timeout = DEFAULT_TCP_EST_TIMEOUT;
    unsigned int tcp_transitory_timeout = DEFAULT_TCP_TRANS_TIMEOUT;
    ...
    while ((c = getopt(argc, argv, "hns:v:p:u:t:r:l:T:I:E:R:")) != EOF)
    {
        switch (c)
        {
            ...
            case 'n':
                ...
                break;
            case 'I':
                icmp_timeout_expiry = atoi((char *) optarg);
                break;
            case 'E':
    ...
    sr_init(&sr);
    /* if nat argument is provided, set teh appropriate args */
    if (useNat) {
      nat.icmp_timeout_expiry = icmp_timeout_expiry;
